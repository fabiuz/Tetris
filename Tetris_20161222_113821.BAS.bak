...
SUB TabuleiroDesenharCelulas(BYREF tabuleiro() AS celulaTabuleiro, BYVAL pecaAtual as peca)
    ...
    FOR linha = 1 TO %CELULAS_POR_LINHA + 1
        ...
        FOR coluna = 1 TO %CELULAS_POR_COLUNA
			...
			' A última linha do tabuleiro, serve como limite inferior da peça, mas usaremos
			' as coordenadas desta linha, para identificar em qual coluna a tabela está.
			' E renderizar a imagem nesta posição.
			if linha = %CELULAS_POR_LINHA + 1 THEN
				' Agora, que percorrermos todas as linhas, já podemos definir marcadores 
				' superiores e inferiores para ajudar o usuário a posicionar a peça.
				...
				if coluna >= pecaAtual.posicao.x_Esquerda and coluna <= pecaAtual.posicao.x_Direita then
					' Vamos descobrir a menor linha, que está indicada na variável
					local colunaTemp as long
					LOCAL menorLinha as long
					for colunaTemp = coluna to pecaAtual.posicao.x_Direita
						if menor_linha_da_coluna_com_celula_valor_um(colunaTemp) < menorLinha then
							menorLinha = menor_linha_da_coluna_com_celula_valor_um(colunaTemp)
						end if
                    next colunatemp
					
					' Desenha a base da esquerda pra direita.
					graphic line(trianguloXEsquerdo, trianguloYInferior)-(trianguloXDireito, trianguloYInferior), %RGB_RED
					
					' Desenha o lado direito, a altura do triângulo será 1 quarto da altura da célula.
					graphic line - (celulaXMeio, celulaYMeio - (celulaYMeio - trianguloYInferior)/2), %RGB_RED
					
					' Desenha o lado esquerdo
					graphic line - (trianguloXEsquerdo, trianguloYInferior), %RGB_RED
					
					' Desenha uma ponta na frente do triângulo.
					graphic line (celulaXMeio, trianguloYSuperior)-(celulaXMeio, celulaYMeio), %RGB_RED					
				eLSE
					' Apaga o triângulo com a cor de fundo 
					
					' Desenha a base da esquerda pra direita.
					graphic line(trianguloXEsquerdo, trianguloYInferior)-(trianguloXDireito, trianguloYInferior), %JOGO_COR_DE_FUNDO
					
					' Desenha o lado direito
					graphic line - (celulaXMeio, celulaYMeio - (celulaYMeio - trianguloYInferior)/2), %JOGO_COR_DE_FUNDO
					
					' Desenha o lado esquerdo
					graphic line - (trianguloXEsquerdo, trianguloYInferior), %JOGO_COR_DE_FUNDO
					
					' Desenha uma ponta na frente do triângulo.
					graphic line (celulaXMeio, trianguloYSuperior)-(celulaXMeio, celulaYMeio), %JOGO_COR_DE_FUNDO	
				end if
